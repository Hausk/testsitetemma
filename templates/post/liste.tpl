...
<body>
        ...
        <ul style="display: flex; flex-direction: column; list-style: none">
            {foreach $posts as $post}
                <li style="width: 25vw; margin: 5px auto; padding: 7px; border: 1px solid black">
                    ...
                    <p style="display: flex; justify-content: space-between;"><span>{$post.content|escape}</span><a href="/post/edition/{$post.id}">M</a>
                </li>
            {/foreach}
        </ul>
        ...
        <h2>Message</h2>
        <form method="POST" action="/post/enregistrer/{$id}">
            <label>Nom</label><br/>
            <input type="text" name="author" placeholder="Nom"><br />
